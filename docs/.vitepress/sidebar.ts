export default {
    '/前端基础/': [
        {
            text: 'HTML5&CSS3',
            collapsed: false,
            items: [
                { text: '前端认识', link: '/前端基础/前端认识.md' },
                { text: 'HTML', link: '/前端基础/HTML.md' },
                { text: 'CSS', link: '/前端基础/CSS.md' },
            ]
        },
        {
            text: 'JavaScript',
            collapsed: false,
            items: [
                { text: 'JS基础', link: '/前端基础/JS基础.md' },
                { text: 'JS高级', link: '/前端基础/JS高级.md' },
                { text: 'JS高级（上）', link: '/前端基础/JS高级（上）.md' },
                { text: 'JS高级（下）', link: '/前端基础/JS高级（下）.md' },
                { text: '案例', link: '/前端基础/案例.md' },
                { text: '闭包练习题', link: '/前端基础/闭包练习题.md' },
            ]
        },
        {
            text: 'TypeScript',
            collapsed: false,
            items: [
                { text: 'TypeScript', link: '/前端基础/TypeScript.md' },
            ]
        },
    ],
    '/前端框架/': [
        {
            text: '前端框架',
            collapsed: false,
            items: [
                { text: 'Vue2', link: '/前端框架/Vue2.md' },
                { text: 'Vue3', link: '/前端框架/Vue3.md' },
                { text: 'React', link: '/前端框架/React.md' },
            ]
        },
        {
            text: '状态管理库',
            collapsed: false,
            items: [
                { text: 'Vuex', link: '/前端框架/Vuex.md' },
                { text: 'Pinia', link: '/前端框架/Pinia.md' },
                { text: '包管理器', link: '/前端框架/包管理工具.md' },
            ]
        }
    ],
    '/小程序/': [
        {
            text: '小程序',
            collapsed: false,
            items: [
                { text: '微信小程序', link: '/小程序/微信小程序.md' },
                { text: 'uniapp', link: '/小程序/uniapp.md' },
                { text: 'Taro', link: '/小程序/Taro.md' },
            ]
        }
    ],
    '/编译&构建&打包/': [
        {
            text: 'Node相关',
            collapsed: false,
            items: [
                { text: 'Node.js', link: '/编译&构建&打包/NodeJs.md' },
            ]
        },
        {
            text: '打包',
            collapsed: false,
            items: [
                { text: 'webpack', link: '/编译&构建&打包/webpack.md' },
            ]
        },
        {
            text: '各种封装',
            collapsed: false,
            items: [
                { text: 'axios封装', link: '/编译&构建&打包/axios封装.md' },
            ]
        },
    ],
    '/组件库/': [
        {
            text: 'CSS 预编译语言',
            collapsed: false,
            items: [
                { text: 'Sass', link: '/组件库/Sass.md' },
                { text: 'Less', link: '/组件库/Less.md' },
            ]
        },
        {
            text: '组件库',
            collapsed: false,
            items: [
                { text: 'Element', link: '/组件库/Element.md' },
                { text: 'Element Plus', link: '/组件库/ElementPlus.md' },
                { text: 'ECharts', link: '/组件库/ECharts.md' },
                { text: '百度地图', link: '/组件库/百度地图.md' },
                { text: '好用插件', link: '/组件库/插件使用.md' },
            ]
        },
        {
            text: 'canvas',
            collapsed: false,
            items: [
                { text: 'canvas', link: '/组件库/canvas.md' },
            ]
        },
    ],
    '/其它/': [
        { text: 'Git', link: '/其它/Git.md' },
        { text: 'sql语句', link: '/其它/sql语句.md' },
        { text: 'mysql封装', link: '/其它/mysql封装.md' },
        {
            text: '好用工具',
            collapsed: false,
            items: [
                { text: '安装教程', link: '/其它/安装教程.md' },
                { text: '好用工具', link: '/其它/好用工具.md' },
            ]
        }
    ],
    '/实战项目/': [
        {
            text: '数据',
            collapsed: false,
            items: [
                { text: '数据处理', link: '/实战项目/数据处理.md' },
            ]
        },
        { text: '文件上传', link: '/实战项目/文件上传.md' },
        { text: '杂七杂八', link: '/实战项目/杂七杂八.md' },
    ],
    '/面试相关/': [
        {
            text: '面试题💫',
            collapsed: false,
            items: [
                { text: 'HTTP', link: '/面试相关/HTTP.md' },
                { text: 'CSS', link: '/面试相关/CSS.md' },
                { text: 'JS', link: '/面试相关/JS.md' },
                { text: 'Vue', link: '/面试相关/Vue.md' },
                { text: '面试记录', link: '/面试相关/面试记录.md' },
            ]
        },
        {
            text: '算法🍨',
            collapsed: false,
            items: [
                { text: '手写代码', link: '/面试相关/手写代码.md' },
                { text: '运行结果', link: '/面试相关/运行结果.md' },
            ]
        },
    ],
    '/杂七杂八/': [
        { text: '搭建博客', link: '/杂七杂八/搭建博客.md' },
        {
            text: '开源项目学习💫',
            collapsed: false,
            items: [
                { text: '开源项目', link: '/杂七杂八/开源项目.md' },
                { text: '博客笔记', link: '/杂七杂八/博客笔记.md' },
            ]
        },
        {
            text: '实际踩坑🕳',
            collapsed: false,
            items: [
                { text: '前期准备', link: '/杂七杂八/前期准备.md' },
                { text: 'git相关', link: '/杂七杂八/git相关.md' },
            ]
        },
    ],
    '/Java/': [
        { text: 'Java入门', link: '/Java/Java入门.md' },
        { text: 'Java基础', link: '/Java/Java基础.md' },
    ],
}